...
func (rt *_router) getUserPhoto(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	if err != nil {
		...
		ctx.Logger.WithError(err).Error("get-user-photo: Error")
		return
	}
	...
	if banned {
		ctx.Logger.WithError(errors.New("l'utente è bloccato")).Error("get-user-profile: Impossibile eseguire l'operazione")
		w.WriteHeader(http.StatusForbidden) //errore 403
		return
	}
